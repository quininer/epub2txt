...
fn app() -> Command {
    Command::new(env!("CARGO_PKG_NAME"))
        ...
        .arg(Arg::new("output").short('o').long("output").value_name("OUTPUT").help("write to file."))
}

fn start(matches: &ArgMatches) -> Result<(), Error> {
    let mut reader = if let Some(path) = matches.get_one::<String>("input").cloned() {
    ...
    let mut writer = if let Some(path) = matches.get_one::<String>("output").cloned() {
